...
As \citet{paixao2015toward} highlight, the fields of population genetics and EC developed largely in parallel for thirty years, independently accumulating overlapping results.
For instance, both  equations that track how the frequencies of different genotypes in a population shift from one generation to the next.
...
In this section, we overview between quantitative gentics, the study of how traits respond to selection, and population genetics, the study of how genetic material flows within species over time and space.
Among many possibilities, several  which may be potentially useful.
...
For diploid populations, when the magnitude of selection is much smaller than one over twice the population size ($|s| \ll 1/(2N)$), drift overwhelms selection and alleles behave as if neutral.  % @MAM could note haploid populations
...
Direct correspondence exist between such analysis and the problem of multi-objective optimization.
...
However, the multivariate breeder's equation has yet to see much application in EC. % @MAM at least, I haven't turned any up though they could exist?
